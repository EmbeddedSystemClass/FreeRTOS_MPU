...
/*
 * Description: In this project we are trying to replicate the Toyota unintended acceleration problem as per the data read,
 * in this project we will be having two independent task acceleration task and braking task, which we communicate with
 * the control task present in the ETCS i.e. electronic throttle control system.
 * A display task is built to show that input was provided by the acceleration task and braking task.
 * We have tried to demonstrate there fail-safe condition and how they have attained it.
 * We have infotainment task added just to add some bugs into the code and point out the flaws of the toyota coding.
 * We have tried to built the same with MPU model to state that such faulty cases can be avoided by it & to make the System robust.
 *
 * Date: 28/01/17
 *
...
//QueueHandle_t xGlobalQueue;   //27.01.17 this queue will support the transaction between ultrasonic sensor--control task and temperature sensor--control task
//QueueHandle_t xGlobalQueue_UART; //27.01.17 this queue will support the transaction between control--uart task.

//this variable will be used to control the motion of the vehicle.
uint32_t run_halt=0; //28.01.17
...
int main( void )
{
	...
	vMyBrakeTaskStart();
	vMyDisplayStart();
	vMyAccelerationTaskStart();
	vMyInfotainmentTaskStart();
