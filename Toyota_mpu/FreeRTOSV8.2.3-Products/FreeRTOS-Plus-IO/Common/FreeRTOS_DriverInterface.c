	...
	switch( ulRequest )
	{
		...
		case ioctlUSE_CIRCULAR_BUFFER_RX :
			...
			#if ioconfigUSE_CIRCULAR_BUFFER_RX == 1
			{
				...
				ulRequest = ioctlUSE_INTERRUPTS;
				//ulRequest = ioctlUSE_RXINTERRUPTS;
				pvValue = ( void * ) pdTRUE;
				xCommandIsDeviceSpecific = pdTRUE;
			}
...
/*			xReturn = xIOUtilsConfigureTaskNotificationBasedRx( pxPeripheralControl, ( unsigned portBASE_TYPE ) pvValue );
		...
		case ioctlUSE_TASK_NOTIFICATION_BASED_TX :
			...
			xReturn = xIOUtilsConfigureTaskNotificationBasedRx( pxPeripheralControl, ulRequest, ( unsigned portBASE_TYPE ) pvValue );
