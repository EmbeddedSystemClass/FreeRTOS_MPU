...
LPC_I2C_T * const pxI2C = ( LPC_I2C_T * const ) diGET_PERIPHERAL_BASE_ADDRESS( pxPeripheralControl ); //24.01.17 This wil hold the base address of the peripheral
I2C_XFER_T *pxI2CTxTransferDefinition = NULL, *pxI2CRxTransferDefinition = NULL; //24.01.17 These Structures are available in the LPC OPEN files.
...
portBASE_TYPE xReturn = pdFAIL;
	...
	if( cPeripheralNumber < boardNUM_I2CS )
	{
		...
		pxI2CTxTransferDefinition = ( I2C_XFER_T * ) pvPortMalloc( sizeof( I2C_XFER_T ) );  //24.01.17 This the data buffer used for the transmission!!
		pxI2CRxTransferDefinition = ( I2C_XFER_T * ) pvPortMalloc( sizeof( I2C_XFER_T ) );  //24.01.17 This the data buffer used for the transmission!!
...
LPC_I2C_T * pxI2C = ( LPC_I2C_T * ) diGET_PERIPHERAL_BASE_ADDRESS( ( ( Peripheral_Control_t * const ) pxPeripheral ) );
	/* Sanity check the array index. */
	configASSERT( cPeripheralNumber < ( int8_t ) ( sizeof( xIRQ ) / sizeof( IRQn_Type ) ) );
