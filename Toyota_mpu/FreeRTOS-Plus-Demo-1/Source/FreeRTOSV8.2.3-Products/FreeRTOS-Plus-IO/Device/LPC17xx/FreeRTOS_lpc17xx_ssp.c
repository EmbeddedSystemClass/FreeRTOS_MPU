 ...
 */
static size_t prvFillFifoFromBuffer( LPC_SSP_T * const pxSSP, uint8_t **ppucBuffer, const size_t xTotalBytes );
...
static SSP_ConfigFormat xSSPConfigurations[ boardNUM_SSPS ];
...
LPC_SSP_T * const pxSSP = ( LPC_SSP_T * const ) diGET_PERIPHERAL_BASE_ADDRESS( pxPeripheralControl );
Chip_SSP_DATA_SETUP_T *pxSSPTransferDefinition;
...
volatile uint16_t usJunkIt;
	...
	if( cPeripheralNumber < boardNUM_SSPS )
	{
		...
		pxSSPTransferDefinition = ( Chip_SSP_DATA_SETUP_T * ) pvPortMalloc( sizeof( Chip_SSP_DATA_SETUP_T ) );
...
LPC_SSP_T * const pxSSP = ( LPC_SSP_T * const ) diGET_PERIPHERAL_BASE_ADDRESS( ( ( Peripheral_Control_t * const ) pxPeripheral ) );
	...
	switch( diGET_TX_TRANSFER_TYPE( pxPeripheralControl ) )
	{
		case ioctlUSE_POLLED_TX :
			...
			#if ioconfigUSE_SSP_POLLED_TX == 1
			{
				...
				pxSSPTransferDefinition = ( Chip_SSP_DATA_SETUP_T * ) diGET_TX_TRANSFER_STATE( pxPeripheralControl );
				...
				Chip_SSP_RWFrames_Blocking(pxSSP, pxSSPTransferDefinition);
			}
	...
	switch( diGET_RX_TRANSFER_TYPE( pxPeripheralControl ) )
	{
		case ioctlUSE_POLLED_RX :
			...
			#if ioconfigUSE_SSP_POLLED_RX == 1
			{
				...
				pxSSPTransferDefinition->length  = ( uint32_t ) xBytes;
				//xReturn = SSP_ReadWrite( pxSSP, pxSSPTransferDefinition, SSP_TRANSFER_POLLING );
				xReturn =Chip_SSP_RWFrames_Blocking(pxSSP, pxSSPTransferDefinition);
			}
...
size_t xBytesSent;
	...
	for( xBytesSent = 0U; ( xBytesSent < sspMAX_FIFO_DEPTH ) && ( xBytesSent < xTotalBytes ); xBytesSent++ )
	{
		...
		if( ( ( pxSSP->SR & SSP_STAT_TNF ) != 0 ) && ( ( pxSSP->SR & SSP_STAT_RFF ) == 0 ) )
		{
			if( *ppucBuffer == NULL )
			{
...
LPC_SSP_T * pxSSP = ( LPC_SSP_T * ) diGET_PERIPHERAL_BASE_ADDRESS( ( ( Peripheral_Control_t * const ) pxPeripheral ) );
	...
	taskENTER_CRITICAL();
	{
		switch( ulRequest )
		{
			case ioctlUSE_INTERRUPTS :
				...
				else
				{
					...
					Chip_SSP_Int_Disable(pxSSP);
					//SSP_IntConfig( pxSSP, sspALL_SSP_RX_INTERRUPTS, ENABLE );
					Chip_SSP_Int_Enable(pxSSP);
			...
			case ioctlSET_SPEED : /* In bits/second. */

				//xSSPConfigurations[ cPeripheralNumber ].ClockRate = ulValue;
				Chip_SSP_SetBitRate(pxSSP, ulValue);
				ulInitSSP = pdTRUE;
				break;
			...
			case ioctlSET_SPI_DATA_BITS	: /* 4 to 16. */
				...
				Chip_SSP_SetFormat(pxSSP, ulValue, 0, 0);
				ulInitSSP = pdTRUE;
				break;
			...
			case ioctlSET_SPI_CLOCK_PHASE_POLARITY: /*SSP_CLOCK_CPHA0_CPOL0 or
			 	 	 	 	 	 	 	 	 	 	  SSP_CLOCK_CPHA0_CPOL1 or
			 	 	 	 	 	 	 	 	 	 	  SSP_CLOCK_CPHA1_CPOL0 or
			 	 	 	 	 	 	 	 	 	 	  SSP_CLOCK_CPHA1_CPOL1*/
				Chip_SSP_SetFormat(pxSSP, 0, 0, ulValue);
			...
			case ioctlSET_SPI_MODE : /* SSP_MASTER_MODE or SSP_SLAVE_MODE. */
				...
				Chip_SSP_Set_Mode(pxSSP, ulValue);
				break;
			...
			case ioctlSET_SSP_FRAME_FORMAT : /* SSP_FRAME_SPI or SSP_FRAME_TI or SSP_FRAME_MICROWIRE. */
				...
				Chip_SSP_SetFormat(pxSSP, 0, ulValue, 0);
				break;
		}
		...
		if( ulInitSSP == pdTRUE )
		{
			...
			Chip_SSP_Disable(pxSSP);
			//SSP_DeInit( pxSSP );
			Chip_SSP_DeInit(pxSSP);
			//SSP_Init( pxSSP, &( xSSPConfigurations[ cPeripheralNumber ] ) );
			Chip_SSP_Init(pxSSP);
			//SSP_Cmd( pxSSP, ENABLE );
			Chip_SSP_DeInit(pxSSP);
...
Transfer_Control_t *pxTxTransferStruct, *pxRxTransferStruct;
	...
	if( ( ulInterruptSource & SSP_RORIC ) != 0 )
	{
		configASSERT( ( ulInterruptSource & SSP_RORIC ) == 0 );
		//LPC_SSP1->ICR = SSP_INTCLR_ROR;
		LPC_SSP1->ICR = SSP_RORIC;
	}

	/* Clear timeouts. */
	if( ( ulInterruptSource & SSP_RTIC ) != 0 )
	{
		//LPC_SSP1->ICR = SSP_INTCLR_RT;
		LPC_SSP1->ICR = SSP_RTIC;
	}

	/* Is this a receive FIFO half full or receive timeout? */
	if( ( ulInterruptSource & (SSP_RTIM | SSP_RXIM) ) != 0 )
	{
		...
		if( pxRxTransferStruct != NULL )
		{
			if( ulReceiveActive[ uxSSPNumber ] == pdFALSE )
			{
				...
				while( ( LPC_SSP1->SR & SSP_STAT_RNE ) != 0 )
				{
					usJunk = LPC_SSP1->DR;
					ulReceived++;
				}
