...
QueueHandle_t xGlobalDispQueue;
...
int main( void )
{
	...
	//SCB->SHCSR|=SCB_SHCSR_BUSFAULTENA_Msk|SCB_SHCSR_MEMFAULTACT_Msk;
	...
	//int foo = NVIC_GetPriority(MemoryManagement_IRQn);
	//foo=NVIC_GetPriority(SVCall_IRQn);


	xGlobalQueue = xQueueCreate( 3, sizeof( xQueueData ) );
	xGlobalDispQueue = xQueueCreate( 3, sizeof( xQueueDisplayData ) );
