...
#if 1			//NGX_MBED_BOARD
#define LCD_CTRL_GPIO_PORT_NUM					2
#define LCD_DATA_GPIO_PORT_NUM					0

#define	LCD_EN_GPIO_PIN_NUM						7
#define	LCD_RS_GPIO_PIN_NUM						5
#define	LCD_RW_GPIO_PIN_NUM						6
#define	LCD_BACKLIGHT_GPIO_PIN_NUM				29
#define	LCD_DATA4_GPIO_PIN_NUM					19
#define	LCD_DATA5_GPIO_PIN_NUM					20
#define	LCD_DATA6_GPIO_PIN_NUM					21
#define	LCD_DATA7_GPIO_PIN_NUM					22
#endif
...
#define LCD_ctrlBusMask ((1<<LCD_RS_GPIO_PIN_NUM)|(1<<LCD_RW_GPIO_PIN_NUM)|(1<<LCD_EN_GPIO_PIN_NUM))
#define LCD_dataBusMask ((1<<LCD_DATA4_GPIO_PIN_NUM)|(1<<LCD_DATA5_GPIO_PIN_NUM)|(1<<LCD_DATA6_GPIO_PIN_NUM)|(1<<LCD_DATA7_GPIO_PIN_NUM))


#define LcdDataBusPort      *(volatile unsigned int *)0x2009C014
#define LcdControlBusPort   *(volatile unsigned int *)0x2009C054
...
/* Baseboard LCD Pins */
#define NUM_LCD_CTRL 4
#define NUM_LCD_DATA 4


#if 1	//NGX_MBED_BOARD
static const uint8_t pinLCDCTRL[NUM_LCD_CTRL] = {
		LCD_RS_GPIO_PIN_NUM,
		LCD_RW_GPIO_PIN_NUM,
		LCD_EN_GPIO_PIN_NUM,
		LCD_BACKLIGHT_GPIO_PIN_NUM
};
#endif


#if 1		//NGX_MBED_BOARD
static const uint8_t pinLCDDATA[NUM_LCD_DATA] = {
		LCD_DATA4_GPIO_PIN_NUM,
		LCD_DATA5_GPIO_PIN_NUM,
		LCD_DATA6_GPIO_PIN_NUM,
		LCD_DATA7_GPIO_PIN_NUM
};
#endif


static void Board_LCD_Send_Nibble(char nibble)
{

	uint32_t state;

	Chip_GPIO_SetPinState(LPC_GPIO,LCD_DATA_GPIO_PORT_NUM,LCD_DATA4_GPIO_PIN_NUM,false);
	Chip_GPIO_SetPinState(LPC_GPIO,LCD_DATA_GPIO_PORT_NUM,LCD_DATA5_GPIO_PIN_NUM,false);
	Chip_GPIO_SetPinState(LPC_GPIO,LCD_DATA_GPIO_PORT_NUM,LCD_DATA6_GPIO_PIN_NUM,false);
	Chip_GPIO_SetPinState(LPC_GPIO,LCD_DATA_GPIO_PORT_NUM,LCD_DATA7_GPIO_PIN_NUM,false);
	//LcdDataBusPort&=~(LCD_dataBusMask);                   // Clear previous data
	state= ((nibble >>0x00) & 0x01);
	Chip_GPIO_SetPinState(LPC_GPIO,LCD_DATA_GPIO_PORT_NUM,LCD_DATA4_GPIO_PIN_NUM,state);
	state= ((nibble >>0x01) & 0x01);
	Chip_GPIO_SetPinState(LPC_GPIO,LCD_DATA_GPIO_PORT_NUM,LCD_DATA5_GPIO_PIN_NUM,state);
	state= ((nibble >>0x02) & 0x01);
	Chip_GPIO_SetPinState(LPC_GPIO,LCD_DATA_GPIO_PORT_NUM,LCD_DATA6_GPIO_PIN_NUM,state);
	state= ((nibble >>0x03) & 0x01);
	Chip_GPIO_SetPinState(LPC_GPIO,LCD_DATA_GPIO_PORT_NUM,LCD_DATA7_GPIO_PIN_NUM,state);
...
void Board_Lcd_Cmd_Write(char cmd)
{
	Board_LCD_Send_Nibble((cmd >> 0x04) & 0x0F);  //Send higher nibble
	Board_lcd_Send_Cmd_Signals();
	delay_ms(10);

	Board_LCD_Send_Nibble(cmd & 0x0F);            //Send Lower nibble
	Board_lcd_Send_Cmd_Signals();
	delay_ms(10);
...
void Board_Lcd_Data_Write(char dat)
{
	Board_LCD_Send_Nibble((dat >> 0x04) & 0x0F);  //Send higher nibble

	Board_lcd_Send_Data_Signals();
	delay_ms(100);

	Board_LCD_Send_Nibble(dat & 0x0F);            //Send higher nibble
	Board_lcd_Send_Data_Signals();
	delay_ms(100);
}

void Board_LCD_Write_String(char *data)
{

	while((*data)!=0)
		Board_Lcd_Data_Write(*data++); // Loop through the string and display char by char

}
static void Board_lcd_Send_Data_Signals(void)
{
	Chip_GPIO_WritePortBit(LPC_GPIO,LCD_CTRL_GPIO_PORT_NUM,LCD_RS_GPIO_PIN_NUM,true);

	Chip_GPIO_WritePortBit(LPC_GPIO,LCD_CTRL_GPIO_PORT_NUM,LCD_RW_GPIO_PIN_NUM,false);

	Chip_GPIO_WritePortBit(LPC_GPIO,LCD_CTRL_GPIO_PORT_NUM,LCD_EN_GPIO_PIN_NUM,true);

	delay_ms(10);
	Chip_GPIO_WritePortBit(LPC_GPIO,LCD_CTRL_GPIO_PORT_NUM,LCD_EN_GPIO_PIN_NUM,false);

}
static void Board_lcd_Send_Cmd_Signals(void)
{

	Chip_GPIO_WritePortBit(LPC_GPIO,LCD_CTRL_GPIO_PORT_NUM,LCD_RS_GPIO_PIN_NUM,false);
...
void Board_LCD_Init(void)
{
	...
	for (ix = 0; ix < NUM_LCD_CTRL; ix++) {
		Chip_GPIO_SetPinDIROutput( LPC_GPIO, LCD_CTRL_GPIO_PORT_NUM, pinLCDCTRL[ix] );
		Chip_GPIO_WritePortBit( LPC_GPIO, LCD_CTRL_GPIO_PORT_NUM, pinLCDCTRL[ix], FALSE );
	}

	for (ix = 0; ix < NUM_LCD_DATA; ix++) {
		Chip_GPIO_SetPinDIROutput( LPC_GPIO, LCD_DATA_GPIO_PORT_NUM, pinLCDDATA[ix] );
		//Chip_GPIO_WritePortBit( LPC_GPIO, portLCDDATA[ix], pinLCDDATA[ix], FALSE );
	}

	Board_Lcd_Cmd_Write(0x33);
	Board_Lcd_Cmd_Write(0x32);
	Board_Lcd_Cmd_Write(0x28);
	Board_Lcd_Cmd_Write(0x0e);
	Board_Lcd_Cmd_Write(0x01);
	Board_Lcd_Cmd_Write(0x06);
	Board_Lcd_Cmd_Write(0x80);
...
{
	/* Sets up DEBUG UART */
	//DEBUGINIT();	//changed according to babu sir's sample
...
void Board_ADC_Init( ADC_CHANNEL_T number )
{
	switch ( number ) {
	case ADC_CH0:
		Chip_IOCON_PinMux(LPC_IOCON, 0, 23, IOCON_MODE_INACT, IOCON_FUNC1 );
		Chip_IOCON_DisableOD(LPC_IOCON, 0, 23 );
		break;

	case ADC_CH1:
		Chip_IOCON_PinMux(LPC_IOCON, 0, 24, IOCON_MODE_INACT, IOCON_FUNC1 );
		Chip_IOCON_DisableOD(LPC_IOCON, 0, 24 );
		break;

	case ADC_CH2:
		Chip_IOCON_PinMux(LPC_IOCON, 0, 25, IOCON_MODE_INACT, IOCON_FUNC1 );
		Chip_IOCON_DisableOD(LPC_IOCON, 0, 25 );
		break;

	case ADC_CH3:
		Chip_IOCON_PinMux(LPC_IOCON, 0, 26, IOCON_MODE_INACT, IOCON_FUNC1 );
		Chip_IOCON_DisableOD(LPC_IOCON, 0, 26 );
		break;

	case ADC_CH4:
		Chip_IOCON_PinMux(LPC_IOCON, 1, 30, IOCON_MODE_INACT, IOCON_FUNC3 );
		Chip_IOCON_DisableOD(LPC_IOCON, 1, 30 );
		break;

	case ADC_CH5:
		Chip_IOCON_PinMux(LPC_IOCON, 1, 31, IOCON_MODE_INACT, IOCON_FUNC3 );
		Chip_IOCON_DisableOD(LPC_IOCON, 1, 31 );
		break;
